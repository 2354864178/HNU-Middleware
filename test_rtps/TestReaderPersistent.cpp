...
TestReaderPersistent::TestReaderPersistent() : mp_participant(nullptr), mp_reader(nullptr), mp_history(nullptr) {}
...
void TestReaderPersistent::run() {
    ...
    std::cout << "[READER] matched=" << m_listener.n_matched.load() << " received=" << m_listener.n_received.load() << std::endl;
}

// 收到新缓存样本时触发：先计数，再把 payload 按 int32 解析出来并打印
void TestReaderPersistent::MyListener::on_new_cache_change_added(RTPSReader* reader, const CacheChange_t* const change) {
    ++n_received;

    int32_t value = 0;
    if (change->serializedPayload.length >= sizeof(value)) {
        std::memcpy(&value, change->serializedPayload.data, sizeof(value));
        std::cout << "[READER] value=" << value << " payload_len=" << change->serializedPayload.length << std::endl;
    } else {
        std::cout << "[READER] payload_len=" << change->serializedPayload.length << " (too short for int32)" << std::endl;
    }
...
void TestReaderPersistent::MyListener::on_reader_matched(RTPSReader*, const MatchingInfo& info) {
