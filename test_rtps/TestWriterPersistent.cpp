...
bool TestWriterPersistent::init() {
  ...
  mp_writer = RTPSDomain::createRTPSWriter(mp_participant, writer_attr, mp_history, &m_listener); // 创建RTPS写入器并关联监听器
...
bool TestWriterPersistent::reg() {
  ...
  return mp_participant->register_writer(mp_writer, topic_desc, writer_qos);  // 注册写入器到参与者，返回注册结果
}
...
void TestWriterPersistent::run(uint16_t samples) {
  ...
  for (uint16_t i = 0; i < samples; ++i) {
    ...
    std::memcpy(ch->serializedPayload.data, &value, sizeof(value)); // 将样本数据复制到缓存更改的序列化负载中
...
void TestWriterPersistent::MyListener::on_writer_matched(
    RTPSWriter*,
