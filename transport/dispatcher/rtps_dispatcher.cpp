...
void RtpsDispatcher::AddReader(const RoleAttributes& self_attr) {
    ...
    // 创建reader的回调函数
    new_reader.reader_listener = std::make_shared<ReaListener>(std::bind(&RtpsDispatcher::OnMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3), self_attr.channel_name);
    // 创建rtps reader history
    new_reader.mp_history = new ReaderHistory(reader_attr.hatt);
    // 创建rtps reader
    new_reader.reader = RTPSDomain::createRTPSReader(participant_->fastrtps_participant(), reader_attr.ratt, new_reader.mp_history, new_reader.reader_listener.get());
    // 注册rtps reader
    bool reg = participant_->fastrtps_participant()->register_reader(new_reader.reader, reader_attr.Tdesc, reader_attr.Rqos);
