...
private:
    ...
    eprosima::fastrdds::rtps::RTPSWriter* rtps_writer;
    eprosima::fastrdds::rtps::WriterHistory* mp_history;
};

template <typename M> RtpsTransmitter<M>::RtpsTransmitter(const RoleAttributes& attr, const ParticipantPtr& participant) : Transmitter<M>(attr), participant_(participant), rtps_writer(nullptr) {}

template <typename M> RtpsTransmitter<M>::~RtpsTransmitter() {
    Disable();
}
...
template <typename M> void RtpsTransmitter<M>::Enable() {
    ...
    // 填充 RtpsWriter 的配置信息
    AttributesFiller::FillInWriterAttr(this->attr_.channel_name, this->attr_.qos_profile, &writer_attr);
...
template <typename M> bool RtpsTransmitter<M>::Transmit(const MessagePtr& msg, const MessageInfo& msg_info) {
    return Transmit(*msg, msg_info);
}

static int a;
template <typename M> bool RtpsTransmitter<M>::Transmit(const M& msg, const MessageInfo& msg_info) {
    if (!this->enabled_) {
