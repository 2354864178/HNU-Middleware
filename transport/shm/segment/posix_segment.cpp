...
bool PosixSegment::OpenOrCreate() {
    ...
    // shm_open创建的共享内存大小默认为0,需要手动设置大小
    // 将共享内存的大小更改成 conf_.managed_shm_size()
    ...
    // 映射共享内存，可读可写 ，
    // 参数说明：
    // 1. 第一个参数为nullptr，表示由系统自动选择映射地址
    // 2. 第二个参数为映射的大小，这里使用conf_.managed_shm_size()，表示映射整个共享内存的大小
    // 3. 第三个参数为映射的权限，这里使用PROT_READ | PROT_WRITE，表示映射区域可读可写
    // 4. 第四个参数为映射的标志，这里使用MAP_SHARED，表示映射区域可以被其他进程共享
    // 5. 第五个参数为共享内存对象的文件描述符，这里使用fd，表示要映射的共享内存对象
    managed_shm_ = mmap(nullptr, conf_.managed_shm_size(), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

    // 如果映射失败
    if (managed_shm_ == MAP_FAILED) {
        // strerror(errno)函数用于获取最近一次系统调用失败时的错误信息，errno是一个全局变量，保存了最近一次系统调用失败时的错误代码。通过strerror函数可以将这个错误代码转换成一个可读的错误消息字符串，方便调试和日志记录。
    ...
    for (; i < conf_.block_num(); ++i) {
        ...
        std::lock_guard<std::mutex> lg(block_buf_lock_); // 在当前作用域内获取并持有互斥锁 block_buf_lock_
    ...
    state_->IncreaseReferenceCounts(); // 增加引用计数，表示有一个消息正在使用这个状态对象
    init_ = true;
    return true;
}
