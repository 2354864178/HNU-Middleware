...
void Participant::Shutdown() {
    ...
    if (fastrtps_participant_ != nullptr) {
        eprosima::fastdds::rtps::RTPSDomain::removeRTPSParticipant(fastrtps_participant_);
...
void Participant::CreateFastRtpsParticipant(const std::string& name, int send_port, eprosima::fastdds::rtps::RTPSParticipantListener* listener) {
    ...
    const char* ip_val = ::getenv("HNU_IP");
    if (ip_val != nullptr) {
        ip_env = ip_val;
        if (ip_env.empty()) {
            AERROR << "invalid HNU_IP (an empty string)";
            return;
        }
    }
    ADEBUG << "hnu ip: " << ip_env;
