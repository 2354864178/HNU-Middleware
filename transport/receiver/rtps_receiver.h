...
template <typename M> RtpsReceiver<M>::RtpsReceiver(const RoleAttributes& attr, const typename Receiver<M>::MessageListener& msg_listener) : Receiver<M>(attr, msg_listener) {
    dispatcher_ = RtpsDispatcher::Instance();
}

template <typename M> RtpsReceiver<M>::~RtpsReceiver() {
    Disable();
}
...
template <typename M> void RtpsReceiver<M>::Enable(const RoleAttributes& opposite_attr) {
...
template <typename M> void RtpsReceiver<M>::Disable(const RoleAttributes& opposite_attr) {
